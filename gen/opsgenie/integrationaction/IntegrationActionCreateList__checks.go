...
func (j *jsiiProxy_IntegrationActionCreateList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*IntegrationActionCreate:
		val := val.(*[]*IntegrationActionCreate)
	...
	case []*IntegrationActionCreate:
		val_ := val.([]*IntegrationActionCreate)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*IntegrationActionCreate; received %#v (a %T)", val, val)
		}
	}
