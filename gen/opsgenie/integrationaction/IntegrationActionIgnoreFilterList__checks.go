...
func (i *jsiiProxy_IntegrationActionIgnoreFilterList) validateAllWithMapKeyParameters(mapKeyAttributeName *string) error {
...
func (j *jsiiProxy_IntegrationActionIgnoreFilterList) validateSetInternalValueParameters(val interface{}) error {
	...
	case *[]*IntegrationActionIgnoreFilter:
		val := val.(*[]*IntegrationActionIgnoreFilter)
	...
	case []*IntegrationActionIgnoreFilter:
		val_ := val.([]*IntegrationActionIgnoreFilter)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*IntegrationActionIgnoreFilter; received %#v (a %T)", val, val)
		}
	}
