...
func (j *jsiiProxy_WorkflowStepsParamBindingsList) validateSetInternalValueParameters(val interface{}) error {
	...
	case cdktf.IResolvable:
		// ok
	case *[]*WorkflowStepsParamBindings:
		val := val.(*[]*WorkflowStepsParamBindings)
	...
	case []*WorkflowStepsParamBindings:
		val_ := val.([]*WorkflowStepsParamBindings)
		val := &val_
		for idx_97dfc6, v := range *val {
			if err := _jsii_.ValidateStruct(v, func() string { return fmt.Sprintf("parameter val[%#v]", idx_97dfc6) }); err != nil {
				return err
	...
	default:
		if !_jsii_.IsAnonymousProxy(val) {
			return fmt.Errorf("parameter val must be one of the allowed types: cdktf.IResolvable, *[]*WorkflowStepsParamBindings; received %#v (a %T)", val, val)
		}
	}
